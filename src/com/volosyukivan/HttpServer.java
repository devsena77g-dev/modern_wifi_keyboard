...
====================================================================================================
src/com/volosyukivan/HttpConnection.java
====================================================================================================
...
//    }
    ...
    // HeaderMatcher stores Content-Type in header[0] and
    // Content-Length in header[1]. Modern browsers normally
    // send a leading space and CRLF after the value, so do not
    // parse the value with the old fixed-position logic.
    byte[] contentLenHeader = header[1];
    int contentLenHeaderLength = headerLen[1];
    int pos = 0;

    while (pos < contentLenHeaderLength
        && (contentLenHeader[pos] == ' '
            || contentLenHeader[pos] == '\t'
            || contentLenHeader[pos] == '\r')) {
      pos++;
    }
    ...
    while (pos < contentLenHeaderLength) {
      ...
      digits++;
      contentLen = (contentLen * 10) + (b - LETTER_ZERO);

      if (contentLen > 1024 * 1024) {
        throw new ConnectionFailureException("form data is too large");
      }
    ...
    if (digits == 0) {
      throw new ConnectionFailureException("invalid Content-Length");
    }
